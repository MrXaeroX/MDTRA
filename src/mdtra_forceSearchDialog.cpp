...
// Purpose:
//	Implementation of MDTRA_ForceSearchDialog
...
MDTRA_ForceSearchDialog :: MDTRA_ForceSearchDialog( QWidget *parent )
							: QDialog( parent )
{
	...
	pFsInfo = new MDTRA_ForceSearchInfo;
	...
	for (int i = 0; i < m_pMainWindow->getProject()->getStreamCount(); i++) {
		...
		if (pStream && pStream->pdb 
			&& g_PDBFormatManager.checkFormat(pStream->format_identifier,PDB_FS_FORCE_X) 
			&& g_PDBFormatManager.checkFormat(pStream->format_identifier,PDB_FS_FORCE_Y) 
			&& g_PDBFormatManager.checkFormat(pStream->format_identifier,PDB_FS_FORCE_Z) ) {
...
MDTRA_ForceSearchDialog :: ~MDTRA_ForceSearchDialog()
{
...
bool MDTRA_ForceSearchDialog :: event( QEvent *ev )
{
...
void MDTRA_ForceSearchDialog :: exec_on_stream_change( void )
{
...
void MDTRA_ForceSearchDialog :: exec_on_selection_editingFinished( void )
{
	update_selection();
	display_selection();
}

static MDTRA_ForceSearchDialog *pFuncSrcObject = NULL;

void forcesearch_print_f( const QString &msg )
{
	if (!pFuncSrcObject) return;
	pFuncSrcObject->sel_labelResults->setText( pFuncSrcObject->sel_labelResults->text().append( msg ) );
}

void forcesearch_select_f( const MDTRA_SelectionSet<MDTRA_PDB_File>* pSet )
{
	if (!pFuncSrcObject) return;
...
void forcesearch_select2_f( const MDTRA_SelectionSet<MDTRA_PDB_File>* pSet )
{
	if (!pFuncSrcObject) return;
...
void MDTRA_ForceSearchDialog :: exec_on_selection_showWholeSelection( void )
{
...
void MDTRA_ForceSearchDialog :: display_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	const MDTRA_Stream *pStream = m_pMainWindow->getProject()->fetchStreamByIndex( streamIndex );
...
void MDTRA_ForceSearchDialog :: update_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	...
	if (!m_pSelectionParser->parse(pStream->pdb, sel_string->text().toAscii(), forcesearch_select_f)) {
		sel_labelResults->clear();
		m_pSelectionParser->printErrors( forcesearch_print_f );
...
void MDTRA_ForceSearchDialog :: update_selection2( void )
{
	...
	QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
	if (!m_pSelectionParser->parse(pStream->pdb, sel_string->text().toAscii(), forcesearch_select2_f)) {
...
void MDTRA_ForceSearchDialog :: build_common_atom_set( void )
{
	int streamIndex1 = sCombo->itemData( sCombo->currentIndex() ).toInt();
	int streamIndex2 = sCombo2->itemData( sCombo2->currentIndex() ).toInt();
...
void MDTRA_ForceSearchDialog :: exec_on_accept( void )
{
	...
	//Define streams
	pFsInfo->streamInfo[0].index = pStream1->index;
	pFsInfo->streamInfo[0].size = m_iSelectionSize;
	pFsInfo->streamInfo[0].data = m_pSelectionData;
	pFsInfo->streamInfo[1].index = pStream2->index;
	pFsInfo->streamInfo[1].size = m_iSelectionSize2;
	pFsInfo->streamInfo[1].data = m_pSelectionData2;

	//Define trajectory fragment to analyze
	pFsInfo->trajectoryMin = 1;
	pFsInfo->trajectoryMax = MDTRA_MIN(pStream1->files.count(), pStream2->files.count());
	if (trajectoryRange->isChecked()) {
		pFsInfo->trajectoryMin = MDTRA_MAX( pFsInfo->trajectoryMin, sIndex->value() );
		pFsInfo->trajectoryMax = MDTRA_MIN( pFsInfo->trajectoryMax, eIndex->value() );
	}

	//Define criterion
	pFsInfo->statCriterion = stat_criterion[sCriterion->currentIndex()].index;
	pFsInfo->bufferDim = stat_criterion[sCriterion->currentIndex()].bufferDim;
	pFsInfo->statDiff = (float)spinCriterionDiff->value();
	pFsInfo->statDot = (float)spinCriterionDot->value();
