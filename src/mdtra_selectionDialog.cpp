...
// Purpose:
//	Implementation of MDTRA_SelectionDialog
...
MDTRA_SelectionDialog :: MDTRA_SelectionDialog( QWidget *parent )
					   : QDialog( parent )
{
	...
	setWindowTitle( tr("Select Atoms") );
...
MDTRA_SelectionDialog :: ~MDTRA_SelectionDialog()
{
...
bool MDTRA_SelectionDialog :: event( QEvent *ev )
{
...
void MDTRA_SelectionDialog :: exec_on_selection_editingFinished( void )
{
	update_selection();
	display_selection();
}

static MDTRA_SelectionDialog *pFuncSrcObject = NULL;

void selection_print_f( const QString &msg )
{
	if (!pFuncSrcObject) return;
	pFuncSrcObject->sel_labelResults->setText( pFuncSrcObject->sel_labelResults->text().append( msg ) );
}

void selection_select_f( const MDTRA_SelectionSet<MDTRA_PDB_File>* pSet )
{
	if (!pFuncSrcObject) return;
...
void MDTRA_SelectionDialog :: exec_on_selection_showWholeSelection( void )
{
...
#if defined(WIN32)
#define SELECTION_LABEL_ITEMS	39
#else
#define SELECTION_LABEL_ITEMS	29
#endif

void MDTRA_SelectionDialog :: display_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	const MDTRA_Stream *pStream = m_pMainWindow->getProject()->fetchStreamByIndex( streamIndex );
...
void MDTRA_SelectionDialog :: update_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	...
	if (!m_pSelectionParser->parse(pStream->pdb, sel_string->text().toAscii(), selection_select_f)) {
		sel_labelResults->clear();
		m_pSelectionParser->printErrors( selection_print_f );
