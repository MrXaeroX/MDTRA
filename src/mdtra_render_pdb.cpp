...
// Purpose:
//	Implementation of MDTRA_Render_PDB_File
...
#define RPDB_FLAG_PROTEIN_BACKBONE	(1<<10)
#define RPDB_FLAG_NUCLEIC_BACKBONE	(1<<11)
#define RPDB_FLAG_NEW_RESIDUE		(1<<12)

//-------------------------------------------------------------------

MDTRA_Render_PDB_File :: MDTRA_Render_PDB_File()
{
	m_iNumAtoms = 0;
	m_iNumSelectedAtoms = 0;
...
void MDTRA_Render_PDB_File :: unload( void )
{
	if (m_pAtoms) {
...
void MDTRA_Render_PDB_File :: reset( void )
{
	m_iNumAtoms = 0;
	m_iNumSelectedAtoms = 0;
	m_iNumBackboneAtoms = 0;
...
void MDTRA_Render_PDB_File :: set_flags( void )
{
	m_iNumBackboneAtoms = 0;
	...
	int residueFlags = 0;
	int last_i = 0;
	MDTRA_Render_PDB_Atom *pLast = NULL;
	MDTRA_Render_PDB_Atom *pAtom = m_pAtoms;

	for (int i = 0; i < m_iNumAtoms; i++, pAtom++) {
		if (pAtom->atomFlags & RPDB_FLAG_NEW_RESIDUE) {
			if (pLast) {
				//previous residue finished
				//now we have its complete flags
				//set the backbone flags for its atoms
				for (int j = last_i; j < i; j++, pLast++) {
					if ((residueFlags & PDB_FLAG_NUCLEIC) && (pLast->atomFlags & RPDB_FLAG_NUCLEIC_BACKBONE)) {
						pLast->atomFlags |= PDB_FLAG_BACKBONE;
						m_iNumBackboneAtoms++;
					} else if (!(residueFlags & PDB_FLAG_WATER) && (pLast->atomFlags & RPDB_FLAG_PROTEIN_BACKBONE)) {
						pLast->atomFlags |= PDB_FLAG_BACKBONE;
						m_iNumBackboneAtoms++;
					}
				}
...
bool MDTRA_Render_PDB_File :: read_atom( int threadnum, unsigned int format, const char *linebuf, MDTRA_Render_PDB_Atom *pOut, int &residue_c )
{
	char title[6];
	...
	//get symbol
	strcpy_s(symbol, 3, UTIL_SymbolForAtomTitle(trimmed_title));
	pOut->color = UTIL_Color4Sym( symbol );
	pOut->radius = MDTRA_GetVdWRadius( symbol );
	...
	if (residue_c != residue_n) {
		residue_c = residue_n;
		pOut->atomFlags = RPDB_FLAG_NEW_RESIDUE;
	}
	...
	if (!_stricmp( trimmed_title, "P" ) ||
		...
		pOut->atomFlags |= RPDB_FLAG_NUCLEIC_BACKBONE;
	}
	else if (!_stricmp( trimmed_title, "C" ) ||
			 !_stricmp( trimmed_title, "O" ) ||
			 !_stricmp( trimmed_title, "N" ) ||
			 !_stricmp( trimmed_title, "CA" )) {
		pOut->atomFlags |= RPDB_FLAG_PROTEIN_BACKBONE;
	}

	return true;
}

bool MDTRA_Render_PDB_File :: load( int threadnum, unsigned int format, const char *filename, int streamFlags )
{
	FILE *fp = NULL;
	char linebuf[82];
	int line_c = 0;
	...
	//allocate atoms
	m_pAtoms = (MDTRA_Render_PDB_Atom*)UTIL_AlignedMalloc(line_c * sizeof(MDTRA_Render_PDB_Atom));
	if (!m_pAtoms) {
		fclose( fp );
		reset();
		return false;
	}
	memset( m_pAtoms, 0, line_c * sizeof(MDTRA_Render_PDB_Atom) );
...
const MDTRA_Render_PDB_Atom* MDTRA_Render_PDB_File :: fetchAtomByIndex( int index ) const
{
	return &m_pAtoms[index];
}

const MDTRA_Render_PDB_Atom* MDTRA_Render_PDB_File :: fetchAtomBySerialNumber( int serialnumber ) const
{
	//fast case
...
void MDTRA_Render_PDB_File :: move_to_centroid( void )
{
	...
	*(int*)&centroid_origin[3] = 0;
	...
	if (!g_bAllowSSE) {
		...
		int sizeof_pdb_atom = sizeof(MDTRA_Render_PDB_Atom);
		...
		*(int*)&inv_num_atoms_vec[3] = 0;
		...
		SSE_PDB_MOVE_TO_CENTROID(MDTRA_Render_PDB_Atom, original_xyz, modified_xyz, PDB_FLAG_BACKBONE);
	}
#endif
}
...
bool MDTRA_Render_PDB_File :: jacobi( float *matrix, float *d, float *v )
{
...
void MDTRA_Render_PDB_File :: eigsrt( float *d, float *v )
{	
	for ( int i = 0; i < 2; i++ ) {
		int k = i;
		float p = d[k];
...
void MDTRA_Render_PDB_File :: align_kabsch( const MDTRA_Render_PDB_File *pOther )
{
	...
	if (!g_bAllowSSE) {
		...
		SSE_PDB_CALC_RTR_MATRIX(MDTRA_Render_PDB_Atom, modified_xyz, PDB_FLAG_BACKBONE);
	}
#endif
...
void MDTRA_Render_PDB_File :: set_selection_flag( int iSelectionCount, const int *iSelectionData )
{
	MDTRA_Render_PDB_Atom *pAt = m_pAtoms;
	const int *pSel = iSelectionData;

	for (int i = 0; i < m_iNumAtoms; i++, pAt++) {
		pAt->atomFlags &= ~PDB_FLAG_SELECTED;
	}

	m_iNumSelectedAtoms = 0;

	for (int j = 0; j < iSelectionCount; j++, pSel++) {
		if (m_pAtoms[*pSel].atomFlags & PDB_FLAG_SELECTED)
			continue;
		m_pAtoms[*pSel].atomFlags |= PDB_FLAG_SELECTED;
		m_iNumSelectedAtoms++;
	}
}
