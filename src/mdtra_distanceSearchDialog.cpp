...
// Purpose:
//	Implementation of MDTRA_DistanceSearchDialog
...
MDTRA_DistanceSearchDialog :: MDTRA_DistanceSearchDialog( QWidget *parent )
							: QDialog( parent )
{
	...
	pDsInfo = new MDTRA_DistanceSearchInfo;
...
MDTRA_DistanceSearchDialog :: ~MDTRA_DistanceSearchDialog()
{
...
bool MDTRA_DistanceSearchDialog :: event( QEvent *ev )
{
...
void MDTRA_DistanceSearchDialog :: exec_on_stream_change( void )
{
...
void MDTRA_DistanceSearchDialog :: exec_on_selection_editingFinished( void )
{
	update_selection();
	display_selection();
}

static MDTRA_DistanceSearchDialog *pFuncSrcObject = NULL;

void distancesearch_print_f( const QString &msg )
{
	if (!pFuncSrcObject) return;
	pFuncSrcObject->sel_labelResults->setText( pFuncSrcObject->sel_labelResults->text().append( msg ) );
}

void distancesearch_select_f( const MDTRA_SelectionSet<MDTRA_PDB_File>* pSet )
{
	if (!pFuncSrcObject) return;
...
void distancesearch_select2_f( const MDTRA_SelectionSet<MDTRA_PDB_File>* pSet )
{
	if (!pFuncSrcObject) return;
...
void MDTRA_DistanceSearchDialog :: exec_on_selection_showWholeSelection( void )
{
...
void MDTRA_DistanceSearchDialog :: display_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	const MDTRA_Stream *pStream = m_pMainWindow->getProject()->fetchStreamByIndex( streamIndex );
...
void MDTRA_DistanceSearchDialog :: update_selection( void )
{
	int streamIndex = sCombo->itemData( sCombo->currentIndex() ).toInt();
	...
	if (!m_pSelectionParser->parse(pStream->pdb, sel_string->text().toAscii(), distancesearch_select_f)) {
		sel_labelResults->clear();
		m_pSelectionParser->printErrors( distancesearch_print_f );
...
void MDTRA_DistanceSearchDialog :: update_selection2( void )
{
	...
	QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
	if (!m_pSelectionParser->parse(pStream->pdb, sel_string->text().toAscii(), distancesearch_select2_f)) {
...
void MDTRA_DistanceSearchDialog :: build_common_atom_set( void )
{
	int streamIndex1 = sCombo->itemData( sCombo->currentIndex() ).toInt();
	int streamIndex2 = sCombo2->itemData( sCombo2->currentIndex() ).toInt();
...
void MDTRA_DistanceSearchDialog :: exec_on_accept( void )
{
	...
	//Define streams
	pDsInfo->streamInfo[0].index = pStream1->index;
	pDsInfo->streamInfo[0].size = m_iSelectionSize;
	pDsInfo->streamInfo[0].data = m_pSelectionData;
	pDsInfo->streamInfo[1].index = pStream2->index;
	pDsInfo->streamInfo[1].size = m_iSelectionSize2;
	pDsInfo->streamInfo[1].data = m_pSelectionData2;

	//Define trajectory fragment to analyze
	pDsInfo->trajectoryMin = 1;
	pDsInfo->trajectoryMax = MDTRA_MIN(pStream1->files.count(), pStream2->files.count());
	if (trajectoryRange->isChecked()) {
		pDsInfo->trajectoryMin = MDTRA_MAX( pDsInfo->trajectoryMin, sIndex->value() );
		pDsInfo->trajectoryMax = MDTRA_MIN( pDsInfo->trajectoryMax, eIndex->value() );
	}

	//Define criterion
	pDsInfo->statCriterion = stat_criterion[sCriterion->currentIndex()].index;
	pDsInfo->bufferDim = stat_criterion[sCriterion->currentIndex()].bufferDim;
	pDsInfo->statMin = (float)spinMin->value();
	pDsInfo->statMax = (float)spinMax->value();
	pDsInfo->statReference = (float)spinCriterion->value();
	pDsInfo->ignoreSameResidue = selIgnore->isChecked();
